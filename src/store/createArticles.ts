...
export default function createArticles(agent, actions, state, setState) {
  ...
  const [articles] = createResource(
    ...
    (args, { value }) => {
      ...
      return agent.Article.bySlug(args[1]).then((article) => ({ ...value, [args[1]]: article }))
    },
    { initialValue: {} }
  )
  ...
  function $req(predicate) {
    ...
    return agent.Article.recents(state.page, LIMIT)
  }
  ...
  Object.assign(actions, {
    ...
    async makeFavorite(slug) {
      ...
      if (article && !article.favorited) {
        ...
        try {
          await agent.Articles.like(slug)
        } catch (err) {
          setState('articles', slug, (s) => ({
            favorited: false,
            favoritesCount: s.favoritesCount - 1
    ...
    async unmakeFavorite(slug) {
      ...
      if (article && article.favorited) {
        ...
        try {
          await agent.Articles.unlike(slug)
        } catch (err) {
          setState('articles', slug, (s) => ({
            favorited: true,
            favoritesCount: s.favoritesCount + 1
