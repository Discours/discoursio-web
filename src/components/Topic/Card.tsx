...
export default (props: TopicProps) => {
  ...
  const [subscribers, setSubscribers] = createSignal<number>(
    (props.topic.topicStat?.subscriptions as number) || 0
  )
    ...
    <div class='topic row'>
      <div class='col-md-7'>
        ...
        <Show when={!props.compact && !body.loading}>
          <div class='topic-description'>{body}</div>
        </Show>
          ...
          <div class='topic-details'>
            ...
            <span class='topic-details__item'>
              {subscribers().toString() +
                ' ' +
                t('follower') +
                plural(subscribers(), locale() === 'ru' ? ['ов', '', 'а'] : ['s', '', 's'])}
            </span>
          </div>
        </Show>
      </div>
