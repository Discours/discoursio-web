...
export default (props: ModalProps) => {
  ...
  const wrapClick = (ev: Event) => { if ((ev.target as HTMLElement).classList.contains('modalwrap')) hideModal() }

  onMount(()=>{
    window.addEventListener('keydown', (e: KeyboardEvent) => { if (e.key === 'Escape') hideModal() })
  })

  return (<Show when={!!modal}>
