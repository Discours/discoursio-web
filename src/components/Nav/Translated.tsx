...
import PageLoadingBar from '../LoadingBar'
  ...
  return (
    <li>
      <NavLink
        ...
        class='inline-flex items-center transition text-[15px] dark:hover:bg-discours-darkLighterBg sm:text-base m-0 sm:m-1 px-3 sm:px-4 py-3 pointer-fine:hover:text-white pointer-fine:hover:bg-discours-medium whitespace-nowrap'
        activeClass='bg-discours-medium dark:bg-discours-light text-white'
...
const LanguageSelector: Component<{ ref: HTMLButtonElement; class?: string }> = (props) => (
  <li class={props.class || ''}>
    <button
      ...
      ref={props.ref}
      class='text-discours-medium dark:brightness-150 focus:color-red-500 bg-no-repeat bg-center px-3 ml-2 w-16'
    >
      <img class='w-full scale-50' src='/img/icons/translate.svg' alt='translate' />
    </button>
  </li>
)
...
const Nav: Component<{ showLogo?: boolean; filled?: boolean }> = (props) => {
  ...
  const navList = createMemo<MenuLinkProps[]>(
    on(
      () => [locale, t('global.nav'), context.topics],
      () => {
        return (t('global.nav') || []).reduce((memo: any, item: any) => {
          ...
          if (item.path == '/topics') {
            if (context.topics?.length) {
              // const direction = context.topics ? t('global.dir', {}, 'ltr') : 'ltr';
              itm.links = context.topics.map(({ title, body, slug }: Partial<Topic>) => ({
                title,
                description: body,
                // direction,
                path: `/topic/${slug}`
  ...
  const Toggle = () => (
    <button
      ...
      class='text-discours-medium dark:brightness-150 focus:color-red-500 bg-no-repeat bg-center hover:border-gray-500 cursor-pointer dark:border-gray-600 dark:hover:border-gray-500 px-3 ml-2 rounded-md h-10 border border-discours-100'
      classList={{
  ...
  return (
    <>
      ...
      <div
        class='sticky top-0 z-50 bg-white dark:bg-discours-darkbg'
        classList={{ 'shadow-md dark:bg-discours-medium': showLogo() }}
      >
        <div class='flex justify-center w-full overflow-hidden'>
          ...
          <nav class='relative px-3 lg:px-12 container lg:flex justify-between items-center max-h-18 z-20'>
            <div
              class={`absolute flex left-0 top-0 bottom-0 left-3 lg:left-12 pr-5 nav-logo-bg ${
                showLogo() ? 'scale-150' : 'scale-0'
              }`}
              ref={logoEl}
            >
              <Link href='/' onClick={onClickLogo} noScroll class={`py-3 flex w-9 `}>
                <span class='sr-only'>Navigate to the home page</span>
                <img class='w-full h-auto z-10' src={'img/logo.png'} alt='logo' />
              </Link>
            </div>
            <ScrollShadow
              class={`group relative nav-items-container ${showLogo() && 'ml-[56px]'}`}
        ...
        >
          <div class='absolute w-full md:w-96 mt-2 md:ml-12 md:mr-5 border dark:border-discours-darkbg rounded-md transition-composite bg-white dark:bg-discours-darkLighterBg shadow-md'>
            <For each={Object.entries(langs)}>
              {([lang, label]) => (
                <button
                  class='first:rounded-t hover:bg-discours-light hover:text-white last:rounded-b border-r p-3 text-sm border-b text-center dark:border-discours-darkbg/70 w-3/6'
                  classList={{
                    'bg-discours-medium text-white': lang == locale(),
                    'hover:bg-discours-light': lang == locale()
                  }}
        ...
        <Show when={subnav().length !== 0}>
          <div
            ...
            class='absolute left-50 bg-gray-200 dark:bg-discours-darkLighterBg shadow-2xl max-w-sm transition duration-750'
            style={{ left: `${screen.width > 768 ? subnavPosition() : 0}px` }}
          >
            <ul class='divide-x divide-transparent flex flex-col'>
              <For each={subnav()}>
                {(link) => (
                  <li class='px-5 hover:bg-discours-default hover:text-white transition duration-300'>
                    <NavLink onClick={() => setSubnav([])} class='px-6 py-5 w-full block' href={link.path}>
                      {link.title}
