<script lang="ts">
	...
	const moreShouts = async () => {
		...
		const r = await fetch('/feed/recents.json?page=' + page)
		if (r.ok) {
    ...
    <div class="page" transition:fade>
        <div class="floor floor--3">
            <div class="wide-container row">
                {#each $shoutslist.slice(lim(start + shoutsOnPage*p), lim(start + shoutsOnPage*p + 3)) as shout}
                    <div class="col-md-4">
                        <ShoutCard {shout} />
                    </div>
                {/each}
        ...
        <div class="floor floor--2">
            <div class="wide-container row">
                    {#each $shoutslist.slice(lim(start + shoutsOnPage*p + 3), lim(start + shoutsOnPage*p + 5)) as shout}
                        <div class="col-md-6">
                            <ShoutCard {shout} />
                        </div>
                    {/each}
        ...
        <div class="floor floor--31">
            <div class="wide-container row">
                <div class="col-md-4">
                    {#each $shoutslist.slice(lim(start + shoutsOnPage*p + 5), lim(start + shoutsOnPage*p + 9)) as shout}
                        <ShoutCard noimage={true} {shout} />
                    {/each}
                </div>
                <div class="col-md-8">
                    <ShoutCard shout={$shoutslist[lim(start + shoutsOnPage*p + 9)-1]} />
                </div>
            </div>
        </div>
        ...
        <div class="floor floor--1">
            <div class="wide-container row">
                <div class="col-md-12">
                    <ShoutCard shout={$shoutslist[lim(start + shoutsOnPage*p + 10)-1]} />
                </div>
            </div>
        </div>
    </div>
